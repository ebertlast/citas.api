...
class SerModel
{
    ...
    public function Get($idafiliado = '', $tipo_doc='', $idmedico = ''){
        try
        {
            $result = array();
            $sql = "EXEC dbo.SPK_SER @IDAFILIADO = ?, @TIPO_DOC = ?, @IDMEDICO = ?";
            $stmt = sqlsrv_prepare($this->db, $sql, array(&$idafiliado, &$tipo_doc, &$idmedico));
            if( !$stmt ) {
                $error="";
